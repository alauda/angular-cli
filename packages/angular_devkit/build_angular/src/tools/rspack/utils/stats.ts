...
import { RspackLoggingCallback } from '@angular-devkit/build-rspack';
import { logging, tags } from '@angular-devkit/core';
import assert from 'node:assert';
import * as path from 'node:path';
import { Configuration, StatsCompilation } from '@rspack/core';
import { Schema as BrowserBuilderOptions } from '../../../builders/browser-rspack/schema';
...
export function statsErrorsToString(
  ...
  for (const error of errors) {
    if (typeof error === 'string') {
      ...
      const message =
        statsConfig.errors || index === -1 ? error.message : error.message.substring(0, index);
...
export function generateBuildEventStats(
  ...
  for (const asset of assets) {
    ...
    if (asset.name.endsWith('.js')) {
      ...
      ngComponentCount += (asset.info as any).ngComponentCount ?? 0;
