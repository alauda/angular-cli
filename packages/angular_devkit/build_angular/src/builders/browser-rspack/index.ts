...
import {
  EmittedFiles,
  RspackFactory,
  RspackLoggingCallback,
  runRspack,
} from '@angular-devkit/build-rspack';
import { imageDomains } from '@ngtools/webpack';
import { Compiler, Configuration, StatsCompilation } from '@rspack/core';
import * as fs from 'fs';
import * as path from 'path';
import { Observable, concatMap, from, map, switchMap } from 'rxjs';
import webpack from 'webpack';
import { getCommonConfig, getStylesConfig } from '../../tools/rspack/configs';
import { markAsyncChunksNonInitial } from '../../tools/rspack/utils/async-chunks';
import { normalizeExtraEntryPoints } from '../../tools/rspack/utils/helpers';
import {
  BuildEventStats,
...
async function initialize(
  ...
  webpackConfigurationTransform?: ExecutionTransformer<Configuration>,
): Promise<{
  ...
  if (webpackConfigurationTransform) {
    transformedConfig = await webpackConfigurationTransform(config as Configuration);
  }
  ...
  return {
    config: (transformedConfig || config) as Configuration,
    projectRoot,
    projectSourceRoot,
    i18n,
  };
  ...
  return from(context.getProjectMetadata(projectName)).pipe(
    ...
    switchMap(
      ...
      ({ config, projectRoot, projectSourceRoot, i18n, cacheOptions }) => {
        ...
        return runRspack(config as Configuration, context, {
          logging:
            transforms.logging ||
            ((stats, config) => {
              if (options.verbose) {
          ...
          concatMap(
            ...
            async (
              ...
              const webpackRawStats = buildEvent.rspackStats;
              if (!webpackRawStats) {
